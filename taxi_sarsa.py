    ...
    def learn_policy(
        ...
        for i in range(num_episodes):
            ...
            while True:
                ...
                if env.step(next_action)[2]:
                    td = reward - self.qtable[state][action]
                else:
                    td = reward + (self.gamma * self.qtable[next_state][next_action]) - self.qtable[state][action]
                self.qtable[state][action] = self.qtable[state][action] + (self.alpha * td)
            ...
            rewards_each_learning_episode.append(t_reward)
